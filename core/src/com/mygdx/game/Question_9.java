...
public class Question_9 extends ScreenAdapter{
	Game game;
	SpriteBatch batch;
	Lives lives;
	Texture question_9;
	...
	public Question_9(Game game) {
		this.game = game;
		batch = new SpriteBatch();
		question_9 = new Texture("question_9.png");
		lives = new Lives();
		skip = new Skip();
		sound = new SoundFx();
	...
	private void update() {
		if (Gdx.input.justTouched()) {
			...
			if (ans4.contains(touchPoint)) {
				...
				game.setScreen(new Question_10(game));
				dispose();
			}
